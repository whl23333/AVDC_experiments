...
class UNetModelChange(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    ...
    def __init__(
        ...
        self.action_Linear = nn.Linear(action_channels, task_token_channels)
...
class NewUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.action_Linear = nn.Linear(action_channels*action_len, task_token_channels)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.task_tokens: # 把t和task embed到一起
            label_emb = y # label_emb: (batch, 512)
            emb = emb + label_emb

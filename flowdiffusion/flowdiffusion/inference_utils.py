...
from .goal_diffusion_change import ActionDecoder, ConditionModel, Preprocess, DiffusionActionModel, SimpleActionDecoder, PretrainDecoder, VQDecoder
from .goal_diffusion_change import DiffusionActionModelWithGPT
from .goal_diffusion_change import DiffusionActionModelWithGPT2
from .goal_diffusion_change import DiffusionActionModelWithGPT3
from .goal_diffusion_change import DiffusionActionModelWithGPT4
from .goal_diffusion_change import DiffusionActionModelWithGPT5
from .goal_diffusion_change import DiffusionActionModelWithGPT6
import os
import yaml
...
def get_video_model_GPT2(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100):
    current_dir = os.path.dirname(__file__)
    config_path = os.path.join(current_dir, '../../configs/config.yaml')
    with open(config_path, "r") as file:
        cfg = yaml.safe_load(file)
    
    unet = Unet_flow() if flow else UnetMWGPT2(action_channels=512)
    pretrained_model = "openai/clip-vit-base-patch32"
    tokenizer = CLIPTokenizer.from_pretrained(pretrained_model)
    text_encoder = CLIPTextModel.from_pretrained(pretrained_model)
    text_encoder.requires_grad_(False)
    ...
    # action_decoder
    action_decoder=PretrainDecoder(dir="../../results12/pretrain/fancy-bush-10",device="cuda")
...
def get_video_model_GPT3(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100):
    current_dir = os.path.dirname(__file__)
    config_path = os.path.join(current_dir, '../../configs/config.yaml')
    with open(config_path, "r") as file:
        cfg = yaml.safe_load(file)
    ...
    # action_decoder
    action_decoder=VQDecoder(dir="../../results12/pretrain/fancy-bush-10", device="cuda")
    ...
    trainer = TrainerChange(
        diffusion_action_model=diffusion_action_model_GPT3,
        tokenizer=tokenizer, 
        text_encoder=text_encoder,
        train_set=[1],
        valid_set=[1],
...
def get_video_model_GPT4(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100):
    current_dir = os.path.dirname(__file__)
    config_path = os.path.join(current_dir, '../../configs/config.yaml')
    with open(config_path, "r") as file:
        cfg = yaml.safe_load(file)
    ...
    trainer = TrainerChange(
        diffusion_action_model=diffusion_action_model_GPT4,
        tokenizer=tokenizer, 
        text_encoder=text_encoder,
        train_set=[1],
        valid_set=[1],
...
def get_video_model_GPT5(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100):
    current_dir = os.path.dirname(__file__)
    config_path = os.path.join(current_dir, '../../configs/config.yaml')
    with open(config_path, "r") as file:
        cfg = yaml.safe_load(file)
    ...
    trainer = TrainerChange(
        diffusion_action_model=diffusion_action_model_GPT5,
        tokenizer=tokenizer, 
        text_encoder=text_encoder,
        train_set=[1],
        valid_set=[1],
...
def get_video_model_GPT6(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100):
    current_dir = os.path.dirname(__file__)
    config_path = os.path.join(current_dir, '../../configs/config.yaml')
    with open(config_path, "r") as file:
        cfg = yaml.safe_load(file)

from .guided_diffusion.guided_diffusion.unet import UNetModel, UNetModelChange, NewUNetModel
...
class UnetMW(nn.Module):
    ...
    def forward(self, x, t, task_embed=None, **kwargs):
        ...
        out = self.unet(x, t, task_embed, **kwargs)
        return rearrange(out, 'b c f h w -> b (f c) h w')
class UnetMWChange(nn.Module):
    def __init__(self):
        super(UnetMWChange, self).__init__()
        self.unet = UNetModelChange(
            image_size=(128, 128),
            in_channels=6,
            model_channels=128,
            out_channels=3,
...
class NewUnetMW(nn.Module):
    def __init__(self):
        super(NewUnetMW, self).__init__()
        self.unet = NewUNetModel(
            image_size=(128, 128),
            in_channels=6,
            model_channels=128,
            out_channels=3,
    ...
    def forward(self, x, t, task_embed=None, **kwargs): # x.shape: (b, (f+1 c), h, w)
        ...
        x_cond = repeat(x[:, -3:], 'b c h w -> b c f h w', f=f)
        x = rearrange(x[:, :-3], 'b (f c) h w -> b c f h w', c=3)
        x = torch.cat([x, x_cond], dim=1) # x.shape=(b (c+c) f h w), 在本任务中，f = 7, c = 3
        out = self.unet(x, t, task_embed, **kwargs)
        return rearrange(out, 'b c f h w -> b (f c) h w')

class UnetMWGPT2(nn.Module):
    def __init__(self, action_channels=512):
        super(UnetMWGPT2, self).__init__()
        self.unet = UNetModelChange(
            image_size=(128, 128),
            in_channels=6,
            model_channels=128,
